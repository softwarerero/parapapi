...
/**
 * Created by IntelliJ IDEA. User: sun Date: 11.12.2010 Time: 13:36:53
 * Copyright.
 */
@Check("admin")
@With(Secure.class)
public class DbHelper {
	...
	public static void backup() throws ClassNotFoundException, SQLException {
		...
		String backupFileName = backupPathName + "/" + datePart
				+ ".parapapi.h2.sql";
...
//		String queryString = "BACKUP TO 'backup.parapapi.zip'";
		...
		org.h2.tools.Script.main("-url", url, "-user", user, "-password", pw,
				"-script", backupFileName); // -url jdbc:h2:~/test -user sa -script
																		// test.zip -options compression zip

		Logger.info("backup'ed to: " + backupFileName);
		st.close();
	}
	...
	public static void executeSqls(String[] sqls) throws ClassNotFoundException,
			SQLException {
